...
const Hero = () => {
  ...
  return <section className="relative h-[600px] overflow-hidden p-0" aria-label="Hero image carousel">
      ...
      align: 'start'
        ...
        <CarouselPrevious 
          className="absolute left-4 top-1/2 -translate-y-1/2 z-10 control-contrast-bg border-none text-white focus:ring-2 focus:ring-white focus:ring-offset-2" 
          aria-label="Go to previous slide"
        />
        
        <CarouselNext 
          className="absolute right-4 top-1/2 -translate-y-1/2 z-10 control-contrast-bg border-none text-white focus:ring-2 focus:ring-white focus:ring-offset-2" 
      ...
      {count > 0 && <div className="absolute inset-x-0 bottom-2 sm:bottom-3 md:bottom-4 z-20 flex justify-center gap-2 pointer-events-none">
          <div className="indicator-contrast-bg px-3 py-1 flex gap-2">
            {Array.from({
          length: count
        }).map((_, index) => <button 
              key={index} 
